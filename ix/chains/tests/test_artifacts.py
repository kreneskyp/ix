...
class TestSaveArtifact:
    def test_create_from_artifact(self, load_chain, ix_handler):
    ...
    def test_create_static_artifact(self, load_chain, ix_handler):
        chain = load_chain(STATIC_ARTIFACT)
        assert isinstance(chain, SaveArtifact)

        result = chain.run(artifact_content=MOCK_CONTENT, callbacks=[ix_handler])
        artifact = Artifact.objects.get()
        assert result == str(artifact.id)
        assert artifact.name == "test artifact"
        assert artifact.key == "test_artifact"
        ...
        assert artifact.storage["type"] == "mock_write_to_file"
        assert (
            artifact.storage["id"] == f"test_artifact_{ix_handler.parent_think_msg.id}"
        )

    @pytest.mark.parametrize("config", [ARTIFACT_FROM_ARTIFACT, STATIC_ARTIFACT])
    def test_artifact_storage_for_artifact(
        self, config, load_chain, tmp_path, ix_handler
    ):
        ...
        result = chain.run(
            artifact_content=MOCK_CONTENT, callbacks=[ix_handler], **kwargs
        )
    ...
    def test_artifact_content_path(self, load_chain, tmp_path, ix_handler):
        """
    ...
    @pytest.mark.parametrize("config", [ARTIFACT_FROM_ARTIFACT, STATIC_ARTIFACT])
    def test_artifact_type(self, config, load_chain, ix_handler):
    ...
    def test_artifact_parent_for_chat_task(self, load_chain, ix_handler):
        """
    ...
    def test_artifact_for_subtask(self, task, ix_handler, load_chain):
        """
        ...
        fake_task(parent=ix_handler.task)
        chain = load_chain(ARTIFACT_FROM_ARTIFACT)
