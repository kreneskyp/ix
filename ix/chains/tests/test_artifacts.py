...
class TestSaveArtifact:
    def test_create_from_artifact(self, mock_callback_manager):
        node = ChainNode.objects.create(**ARTIFACT_FROM_ARTIFACT)
    ...
    def test_create_static_artifact(self, mock_callback_manager):
        node = ChainNode.objects.create(**STATIC_ARTIFACT)
        chain = node.load_chain(mock_callback_manager)
        assert isinstance(node, ChainNode)
        assert isinstance(chain, SaveArtifact)
    ...
    def test_artifact_parent_for_chat_task(self, mock_callback_manager):
        """
    ...
    def test_artifact_for_subtask(self, mock_callback_manager):
        """
        ...
        node = ChainNode.objects.create(**ARTIFACT_FROM_ARTIFACT)
        chain = node.load_chain(subtask_manager)
